...
export class GitHubWebhookService {
  ...
  async handlePushEvent(payload: any): Promise<void> {
    this.logger.log(
      `Processing push event for repository: ${payload.repository.full_name}`,
    );
  ...
  async handlePullRequestEvent(payload: any): Promise<void> {
    this.logger.log(
      `Processing pull request event for repository: ${payload.repository.full_name}`,
    );
  ...
  private async handleMergedPR(
    repository: string,
    pullRequest: any,
  ): Promise<void> {
    this.logger.log(
      `Processing merged PR #${pullRequest.number} in ${repository}`,
    );
  ...
  private async analyzePRChanges(
    ...
    this.logger.log(
      `Analyzing PR #${pullRequest.number} changes in ${repository}`,
    );

    // This is where we could implement PR change analysis
    // For now, we'll log the event for future implementation
    this.logger.log(
      `PR #${pullRequest.number} - Head: ${pullRequest.head.sha}, Base: ${pullRequest.base.sha}`,
    );
  ...
  private async processCommitChanges(
    ...
    for (const filePath of added) {
      const affectedReferences = this.findReferencesForFile(
        allReferences,
        filePath,
      );
  ...
  private async createChangeEvent(eventData: {
    ...
    this.logger.log(
      `Created change event for ${eventData.filePath} in ${eventData.repository}`,
    );
  }
  ...
  async processChangeEvent(changeEvent: CodeChangeEvent): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to process change event ${changeEvent.id}:`,
        error,
      );
