...
describe('NotificationService', () => {
  ...
  describe('sendChangeNotification', () => {
    it('should send notification for code change', async () => {
      ...
      mockSlackService.sendCodeChangeNotification.mockResolvedValue({
        ok: true,
      });
      ...
      expect(mockSlackService.sendCodeChangeNotification).toHaveBeenCalledWith({
        ...
        message: expect.stringContaining(
          'Code reference ref1 has been modified',
        ),
      });
    });
    ...
    it('should handle notification failure gracefully', async () => {
      ...
      mockSlackService.sendCodeChangeNotification.mockRejectedValue(
        new Error('Slack error'),
      );

      // Should not throw
      await expect(
        service.sendChangeNotification(payload),
  ...
  describe('sendConflictNotification', () => {
    it('should send notification for code conflict', async () => {
      ...
      expect(mockSlackService.sendCodeChangeNotification).toHaveBeenCalledWith({
        ...
        resolution: 'manual',
        message: expect.stringContaining(
          'Code conflict detected for reference ref1',
        ),
      });
    });
  ...
  describe('formatChangeMessage', () => {
    it('should format message for modified code', () => {
      const message = service.formatChangeMessage(
        'ref1',
        'modified',
        'old',
        'new',
  ...
  describe('formatConflictMessage', () => {
    it('should format message for conflict with manual resolution', () => {
      const message = service.formatConflictMessage(
        'ref1',
        'deleted',
        'manual',
      );
      ...
      expect(message).toContain(
        'Resolution required: Manual intervention needed',
      );
    });
  ...
  describe('sendBulkNotifications', () => {
    it('should send multiple notifications in batch', async () => {
      ...
      expect(mockSlackService.sendCodeChangeNotification).toHaveBeenCalledTimes(
        2,
      );
    });
