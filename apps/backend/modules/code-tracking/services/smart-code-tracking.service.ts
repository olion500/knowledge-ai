...
export class SmartCodeTrackingService {
  ...
  private async handleModifiedFile(
    ...
    this.logger.log(
      `Handling modified file for reference: ${codeReference.id}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Failed to get file content for ${codeReference.filePath}:`,
        error,
      );
      throw error;
    }
  ...
  async updateCodeReference(
    ...
    if (codeReference.functionName && !codeReference.startLine) {
      ...
      if (!functionSignature) {
        this.logger.warn(
          `Function ${codeReference.functionName} not found in updated file`,
        );
        return;
      }
    ...
    if (newStartLine && newEndLine) {
      newContent = this.extractCodeSnippet(
        fileContent,
        newStartLine,
        newEndLine,
      );
    ...
    if (newContentHash === codeReference.contentHash) {
      this.logger.log(
        `No content change detected for reference: ${codeReference.id}`,
      );
      return;
    }
    ...
    if (codeReference.content && newContent !== codeReference.content) {
      ...
      if (movementResult.found && movementResult.confidence > 0.8) {
        this.logger.log(
          `Detected line movement for reference: ${codeReference.id}`,
        );
        if (movementResult.newStartLine && movementResult.newEndLine) {
          ...
          newContent = this.extractCodeSnippet(
            fileContent,
            movementResult.newStartLine,
            movementResult.newEndLine,
          );
          codeReference.updateLineNumbers(newStartLine, newEndLine);
        }
      }
  ...
  async detectFunctionSignature(
    ...
    const patterns = [
      ...
      new RegExp(
        `(?:public|private|protected|static)?\\s*${functionName}\\s*\\(([^)]*)\\)(?:\\s*:\\s*([^{]+))?\\s*{`,
        'g',
      ),
      // Arrow functions
      new RegExp(
    ...
    const functionContent = this.extractCodeSnippet(
      fileContent,
      startLine,
      endLine,
    );
  ...
  async detectLineMovement(
    oldContent: string,
    newFileContent: string,
  ): Promise<CodeMovementResult> {
