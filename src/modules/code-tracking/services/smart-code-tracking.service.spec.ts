...
describe('SmartCodeTrackingService', () => {
  ...
  beforeEach(async () => {
    ...
    codeExtractorService = module.get<CodeExtractorService>(CodeExtractorService);
  ...
  describe('processCodeChange', () => {
    it('should process modified files successfully', async () => {
      ...
      mockCodeExtractorService.getFileContent.mockResolvedValue('new file content');
    ...
    it('should handle deleted files', async () => {
      ...
      mockNotificationService.sendConflictNotification.mockResolvedValue(undefined);

      const result = await service.processCodeChange(changeEvent);

      expect(result).toBe(true);
      expect(codeReference.markAsDeleted).toHaveBeenCalled();
      expect(mockCodeReferenceRepository.save).toHaveBeenCalledWith(codeReference);
      expect(mockNotificationService.sendConflictNotification).toHaveBeenCalled();
    });
  ...
  describe('updateCodeReference', () => {
    it('should update reference when content changes', async () => {
      ...
      jest.spyOn(service, 'extractCodeSnippet').mockReturnValue('new code snippet');
      jest.spyOn(service, 'calculateContentHash').mockReturnValue('new-hash');
      mockCodeReferenceRepository.save.mockResolvedValue(codeReference);
      mockNotificationService.sendChangeNotification.mockResolvedValue(undefined);

      await service.updateCodeReference(codeReference, fileContent);

      expect(codeReference.updateContent).toHaveBeenCalledWith('new code snippet', 'new-hash');
      expect(mockCodeReferenceRepository.save).toHaveBeenCalledWith(codeReference);
    ...
    it('should handle function-based references', async () => {
      ...
      jest.spyOn(service, 'detectFunctionSignature').mockResolvedValue(functionSignature);
      jest.spyOn(service, 'extractCodeSnippet').mockReturnValue('function code');
      jest.spyOn(service, 'calculateContentHash').mockReturnValue('new-hash');
      mockCodeReferenceRepository.save.mockResolvedValue(codeReference);

      await service.updateCodeReference(codeReference, fileContent);

      expect(service.detectFunctionSignature).toHaveBeenCalledWith(fileContent, 'testFunction');
      expect(codeReference.updateLineNumbers).toHaveBeenCalledWith(5, 10);
      expect(codeReference.updateContent).toHaveBeenCalledWith('function code', 'new-hash');
    });
  ...
  describe('detectFunctionSignature', () => {
    it('should detect TypeScript function signature', async () => {
      ...
      const result = await service.detectFunctionSignature(fileContent, 'testMethod');
  ...
  return a + b;
      ...
      const result = await service.detectFunctionSignature(fileContent, 'testFunction');
    ...
    it('should return null for non-existent function', async () => {
      ...
      const result = await service.detectFunctionSignature(fileContent, 'nonExistentFunction');
  ...
  describe('detectLineMovement', () => {
    it('should detect moved code with high confidence', async () => {
      ...
      const result = await service.detectLineMovement(oldContent, newFileContent);
