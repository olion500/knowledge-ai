...
import { NotificationPayload, ConflictResolution } from '../../../common/interfaces/code-tracking.interface';

@Injectable()
export class NotificationService {
  private readonly logger = new Logger(NotificationService.name);

  constructor(@Inject('SlackService') private readonly slackService: SlackService) {}
  ...
  async sendChangeNotification(payload: NotificationPayload): Promise<void> {
    try {
      ...
      this.logger.log(`Sent change notification for reference: ${payload.referenceId}`);
    } catch (error) {
      this.logger.error(`Failed to send change notification for ${payload.referenceId}:`, error);
  ...
  async sendConflictNotification(payload: NotificationPayload): Promise<void> {
    if (!payload.conflictResolution) {
      this.logger.warn('No conflict resolution provided for conflict notification');
      return;
    }
    ...
    try {
      ...
      this.logger.log(`Sent conflict notification for reference: ${payload.referenceId}`);
    } catch (error) {
      this.logger.error(`Failed to send conflict notification for ${payload.referenceId}:`, error);
      // Don't throw - notifications should not break the main flow
    }
  }
  ...
  async sendBulkNotifications(payloads: NotificationPayload[]): Promise<void> {
    ...
    const notifications = payloads.map(async (payload) => {
      try {
        ...
        this.logger.error(`Failed to send notification for ${payload.referenceId}:`, error);
  ...
  async sendRepositoryChangesSummary(
    ...
    try {
      const message = `📊 Repository Update Summary\n\n` +
      ...
      this.logger.error(`Failed to send repository changes summary for ${repository}:`, error);
    }
  }
} 