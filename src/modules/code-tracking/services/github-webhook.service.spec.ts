...
describe('GitHubWebhookService', () => {
  ...
  beforeEach(async () => {
    ...
    smartTrackingService = module.get<SmartCodeTrackingService>(SmartCodeTrackingService);
  });
  ...
  describe('processChangeEvent', () => {
    it('should process a change event successfully', async () => {
      ...
      expect(mockSmartTrackingService.processCodeChange).toHaveBeenCalledWith(changeEvent);
    ...
    it('should handle processing failure', async () => {
      ...
      expect(changeEvent.markAsProcessing).toHaveBeenCalled();
      expect(changeEvent.markAsFailed).toHaveBeenCalledWith('Processing failed');
  ...
  describe('findAffectedReferences', () => {
    it('should find references for given repository and file paths', async () => {
      ...
      const result = await service.findAffectedReferences('owner/repo', ['file1.ts', 'file3.ts']);
    ...
    it('should return empty array when no references found', async () => {
      ...
      const result = await service.findAffectedReferences('owner/repo', ['file1.ts']);
  ...
  describe('validateWebhookSignature', () => {
    ...
    it('should reject incorrect signature', () => {
      ...
      const result = service.validateWebhookSignature(payload, wrongSignature, secret);
